// From: https://github.com/denoland/deno/pull/1039/files
// Utility to join / resolve a path given a base URI
// Originally written by Deno authors, adapted for Rust 2018
...
pub fn uri_join(base_uri: uri::Uri, ref_str: &str) -> Result<uri::Uri, uri::InvalidUriParts> {
    ...
    if ref_uri.scheme() != None {
        ...
        let joined_path_and_query =
            uri::PathAndQuery::from_maybe_shared::<String>(joined_path.into()).unwrap();
